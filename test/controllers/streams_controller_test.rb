...
class StreamsControllerTest < ActionDispatch::IntegrationTest
  ...
  test "should create stream" do
    assert_difference('Stream.count') do
      post streams_url, params: { stream: { title: @stream.title, user_id: @stream.user_id } }
    end
  ...
  test "should update stream" do
    patch stream_url(@stream), params: { stream: { title: @stream.title, user_id: @stream.user_id } }
